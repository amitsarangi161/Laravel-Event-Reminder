...
class SendRemindersCommand extends Command
{
    ...
    public function handle()
    {
        $count = 0;
        $times = [
            now('Asia/Calcutta')->subDay(),
            now('Asia/Calcutta')->addDay()
        ];
        ...
        foreach ($events as $event) {
            //dd(now($event->timezone)->diffInMinutes($event->start_time, false),now($event->timezone)->diffInMinutes($event->start_time, false));
